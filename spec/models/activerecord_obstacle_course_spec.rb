...
# ========================
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = User.joins(:order_items).where('order_items.item_id = ?', @item_8.id).group(:user_id).order(:name).pluck(:name)
    # ------------------------------------------------------------
...
# ========================
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    ordered_items_names = Item.joins(:order_items).group(:item_id).pluck(:name)
    # ---------------------------------------------------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:orders).select("users.*, count(orders.id) as total_order_count").group(:name).order("total_order_count")
    # ---------------------------------------------------------------
  ...
  xit '28. returns a table of information for all users items' do
    ...
    expect(custom_results[2].name).to eq(@user_1.name)
    expect(custom_results[2].total_item_count).to eq(14)
  end
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.joins(orders: :order_items).select("users.name as user_name, orders.id as order_id, orders.amount / count(order_items.id) as avg_item_cost").group(:name, "order_id").order("user_name desc, avg_item_cost asc")
    # ---------------------------------------------------------------
